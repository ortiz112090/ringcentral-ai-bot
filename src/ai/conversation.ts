...
import { anthropic, firstTextBlock } from "./anthropicClient";
import { retrieveRelevantLessons } from "./retrieval";
import { LeadRecord } from "../db/types";

/** The structured decision Claude returns each turn. */
...
/**
 * Runs one turn of the conversation through Claude and parses the JSON control block.
...
export async function getBotDecision(
  ...
  try {
    ...
    const response = await anthropic.messages.create({
      ...
      max_tokens: 500,
      system: buildSystemPrompt(lead, lessons),
      messages: history.map((t) => ({ role: t.role, content: t.content })),
    });

    return parseDecision(firstTextBlock(response.content));
  } catch (err) {
    logger.error("Claude call failed; falling back to escalation", {
...
/** Extracts and validates the JSON control block from Claude's text output. */
export function parseDecision(raw: string): BotDecision {
  try {
    // Claude is told to return pure JSON, but be tolerant of accidental fencing/prose.
