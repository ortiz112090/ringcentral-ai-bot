...
function fakeReq(opts: { headers?: Record<string, string>; body?: unknown }) {
...
const auth = { "Verification-Token": "rc-token" };
...
describe("handleRcSmsWebhook — validation handshake", () => {
  it("echoes the Validation-Token header with 200 and does nothing else", async () => {
    const res = fakeRes();
    await handleRcSmsWebhook(fakeReq({ headers: { "Validation-Token": "abc123" } }), res);
...
describe("handleRcSmsWebhook — fail-closed auth", () => {
  it("503 when RC_SMS_WEBHOOK_TOKEN is unset", async () => {
    ...
    const res = fakeRes();
    await handleRcSmsWebhook(fakeReq({ headers: auth, body: inboundEvent() }), res);
    expect(res.statusCode).toBe(503);
    expect(handleInboundSms).not.toHaveBeenCalled();
  });

  it("403 on a wrong verification token", async () => {
    const res = fakeRes();
    await handleRcSmsWebhook(fakeReq({ headers: { "Verification-Token": "nope" }, body: inboundEvent() }), res);
    expect(res.statusCode).toBe(403);
    expect(handleInboundSms).not.toHaveBeenCalled();
  });

  it("403 on a missing verification token", async () => {
    const res = fakeRes();
...
describe("handleRcSmsWebhook — routing into the shared pipeline", () => {
  ...
  it("ignores non-inbound (outbound echo) events", async () => {
    const res = fakeRes();
    await handleRcSmsWebhook(fakeReq({ headers: auth, body: inboundEvent({ direction: "Outbound" }) }), res);
    expect(res.statusCode).toBe(200);
    expect(handleInboundSms).not.toHaveBeenCalled();
  });
  ...
  it("dedupes a redelivered message id (in-memory LRU) — processes once", async () => {
    const res1 = fakeRes();
    await handleRcSmsWebhook(fakeReq({ headers: auth, body: inboundEvent() }), res1);
    const res2 = fakeRes();
    await handleRcSmsWebhook(fakeReq({ headers: auth, body: inboundEvent() }), res2);
  ...
  it("destination mismatch (to != rc_sms_number) → 200, no pipeline call (fail closed)", async () => {
    ...
    await handleRcSmsWebhook(
      fakeReq({ headers: auth, body: inboundEvent({ to: [{ phoneNumber: "+19998887777" }] }) }),
      res
    );
    expect(res.statusCode).toBe(200);
    expect(handleInboundSms).not.toHaveBeenCalled();
